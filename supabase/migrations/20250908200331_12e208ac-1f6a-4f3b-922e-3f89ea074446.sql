-- Fix stock transaction trigger to handle guest accounts properly
-- The issue is that guest consumptions might use a transaction_type that doesn't match the enum

-- First, let's check what transaction types are allowed
-- ALTER TYPE if needed, but for now let's make sure our trigger uses correct values
...
BEGIN
  ...
  IF item_category = 'mixed_drinks' THEN
    ...
    LOOP
      ...
      VALUES (
        ...
        'consumption', -- Use 'consumption' instead of 'sale'
  ...
  ELSE
    ...
    INSERT INTO public.stock_transactions (item_id, quantity_change, transaction_type, notes, created_by)
    VALUES (NEW.item_id, -1, 'consumption', 'Automatic stock decrease from consumption', NEW.user_id);
  END IF;
