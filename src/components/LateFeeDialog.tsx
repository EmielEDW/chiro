...
const LateFeeDialog = ({ onLateFeeProcessed, children }: LateFeeDialogProps) => {
  ...
  const calculateLateFee = (minutesLate: number): number => {
    ...
    if (minutesLate <= 4) return 100; // 1 euro
    return Math.min(Math.floor((minutesLate - 1) / 5) + 1, 5) * 100; // Max 5 euros
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          user_id: user.id,
          item_id: '00000000-0000-0000-0000-000000000000', // Dummy item ID for late fees
